...
type lo_gridsim_info
    ...
    integer :: dim_a=-lo_hugeint
    integer :: dim_c=-lo_hugeint
...
subroutine initialize_gridsim(gs,filename,map,order,temperaturescale,distancescale,pairfittype,weighted,verbosity,mw)
    ...
    checkfile: block
        ...
        ! Figure out if we have temperature,volume,eta,a,or c.
        ...
        do i=1,gs%ndim
            ...
            if ( trim( gs%info%dimension_names(i) ) .eq. 'a' )   gs%info%dim_a=i
            if ( trim( gs%info%dimension_names(i) ) .eq. 'c' )   gs%info%dim_c=i
        enddo
        ...
        ! Convert a and c to atomic units (Angstrom to Bohr)
        if ( gs%info%dim_a .gt. 0 ) gs%grid_coordinates(gs%info%dim_a,:)=gs%grid_coordinates(gs%info%dim_a,:)*lo_A_to_bohr
        if ( gs%info%dim_c .gt. 0 ) gs%grid_coordinates(gs%info%dim_c,:)=gs%grid_coordinates(gs%info%dim_c,:)*lo_A_to_bohr
